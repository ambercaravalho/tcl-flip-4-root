...
class usb_class(DeviceClass):
    ...
    def usbread(self, resplen=None, timeout=0):
        if timeout == 0:
            timeout = 1
        # A finite timeout is mandatory: with None the underlying libusb read
        # can block forever when the device is silent, hanging the whole tool.
        if timeout is None:
        ...
        while len(res) < resplen:
            try:
                ...
                if "timed out" in error:
                    # Bounded retries so a silent device can't hang forever.
                    # (The original code compared the timeout value against 10,
                    # which never matched, causing an infinite retry loop.)
                    timeouts += 1
                    if timeouts >= 10:
                        break
                    continue
